...
class PromotionModel(Model):
    """
    Represents a promotion with attributes such as ID, description, discount, start time, end time, status, reservation, and created by.
    """
...
class ReservationModel(Model):
    """
    Represents a reservation with attributes such as ID, reserved by, workspace, start time, end time, status, and price.
    """
    id = AutoField()
...
class ScheduleModel(Model):
    """
    Represents a schedule with attributes such as ID, opening time, closing time, and status.
    """
    id = AutoField()
...
class WorkspaceModel(Model):
    """
    represents a workspace with attributes such as ID, type, capacity, hourly rate, available schedules, and created by.
    """
    id = AutoField()
    type = CharField()
    capacity = CharField()
