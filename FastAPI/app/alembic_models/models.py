...
class Reservation(Base):
    ...
    id = Column(Integer, primary_key=True)
    reservedBy = Column(Integer, ForeignKey('person.id'), nullable=False)  # Relaciona con Person
    workspace_id = Column(Integer, ForeignKey('workspace.id'), nullable=False)  # Relaciona con Workspace
    startTime = Column(String(250), nullable=False)
    endTime = Column(String(250), nullable=False)
    status = Column(String(250), nullable=False)
...
class Workspace(Base):
    ...
    createdBy = Column(String(250), nullable=False)  # Puede referirse a un admin que creó el espacio
